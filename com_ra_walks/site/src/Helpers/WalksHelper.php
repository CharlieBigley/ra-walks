...
//        echo $sql;
    ...
    public function countPhotos($id = 0) {
         ...
         * This counts actual number of photos that are present
          $folder = JPATH_BASE . '/' . rtrim($this->itemomponentHelper::getParams('com_ra_wf')->get('walks_folder'), '/') . '/';
...
// don't allow an existing contact name to be deleted
        if (!trim($newValue) == "") {
            $this->item->contact_display_name = trim(substr($newValue, 0, 50));
        }
    }
...
//        echo "BuildLink: url = $url, " . substr($url, 0, 4) . ", text=$text<br>";
        ...
        if (substr($url, 0, 4) == "http") {
            ...
            $website_base = rtrim(ComponentHelper::getParams('com_ra_wf')->get('website'), '/') . '/';
            $out .= $website_base;
            echo 'base ' . $out . '<br>';
        }
        $out .= $url . $q;
...
//        echo "checkExists: id=" . $this->id . "<br>";
    ...
    function checkLeader() {
        if (trim($this->item->contact_display_name) == "") {
            return 0;
        }
...
//            echo $this->item->contact_display_name . " gives user_id " . $id;
            return $id;
        }
    }
    ...
    public function encode($user_id, $mode) {
         ...
         */
//        echo "<br>date=" . $this->item->walk_date . ",id=" . $this->walk_id . ",mode=$mode<br>";
...
// add any required leading zeroes to give 8 characters in length
        ...
        if ($mode == "F") {                          // Feedback
            if ($user_id == $this->item->leader_user_id) { // this is the leader of the walk
                $part1 .= "04";
...
// Insert values.
        $values = array($id, Factory::getUser()->id);
...
// generates a link to Walkfinder for the specified Walk
    ...
    function getWalkDetails() {
        $message = $this->item->description . '<br>';
        $message .= $this->item->notes . '<br>';
        $message .= "Group: " . $this->item->group_code . ' ' . $this->getGroupname();
        $message .= '<br>';
        if ($details = $this->meetingDescription()) {
            $message .= "Meet: " . $details . '<br>';
            if (($this->item->meeting_latitude == 0) or ($this->item->meeting_latitude == 0)) {
        ...
        if ($details = $this->startDescription()) {
            $message .= "Start: " . $details . '<br>';
            if (($this->item->start_latitude == 0) or ($this->item->start_longitude == 0)) {
        ...
        if (!$details == "") {
            ...
            $message .= $this->item->ascent_metres . " metres" . '<br>';
        }
    ...
    function get_contact_details(&$message) {
        $message = $this->item->contact_tel1;
//        if ($message == "") {
//            return 0;
//        }
        echo 'get_contact_details ' . Factory::getUser()->id;
        if (!$this->item->contact_tel2 == "") {
            if ($message == "") {
                $message = $this->item->contact_tel2;
            } else {
                $message .= "," . $this->item->contact_tel2;
            }
        }
...
//        echo $this->item->walk_date;
//        echo ', ' . date('Y-m-d H:i:s') . '<br>';
        $walk_date = date_create($this->item->walk_date);
        $now = date_create(date('Y-m-d'));
...
//        echo $interval->format('%R%a');
    ...
    function getDate($option = 0) {
        ...
        $date = date_create($this->item->walk_date);
        $pretty = '';
        if ($option >= 2) {
...
// However, the walk leader will always see the names in full
        ...
        if ($this->item->count_followers == 0) {
            $list .= '<li>(none)</li>';
        } else {
...
//                }
    ...
    function getFollowers() {
        if ($this->countFollowers() > 0) {
            ...
            $sql .= "profile.groups_to_follow, ";
            $sql .= "profile.home_group, ";
            $sql .= "users.email AS 'email', ";
            $sql .= "profile.racknowledge_follow AS 'acknowledge_follow', ";
            $sql .= "profile.rprivacy_level AS 'privacy_level', ";
            $sql .= "profile.contactviaemail AS 'contactviaemail', "; //
            $sql .= "profile.mobile AS 'mobile', ";
            $sql .= "profile.contactviatextmessage AS 'contactviatextmessage', ";
            $sql .= "walk_follow.id, ";
...
// Find user from the contact details on the walk
        if ($this->item->leader_user_id == 0) {
            return $this->item->contact_email;
        } else {
            $objProfile = new Ra_toolsProfile;
            $objProfile->id = $this->item->leader_user_id;
    ...
    function getTitle() {
        $message = "Walk led by " . $this->item->contact_display_name;
    ...
    function meetingDescription() {
        $message = '';
        if ($this->item->meeting_time == "") {
            return false;
        }
        $message .= ", " . $this->item->meeting_details;
        $message .= ", " . $this->item->meeting_postcode . ',';
        $message .= ", " . $this->item->meeting_gridref;
        $message .= $this->getStreetmapLink($this->item->meeting_gridref, $this->item->meeting_latitude, $this->item->meeting_longitude);
        if ($this->item->meeting_latitude != 0) {
            $message .= "Lat: " . $this->item->meeting_latitude;
        }
        if ($this->item->meeting_longitude != 0) {
            $message .= ", Lon: " . $this->item->meeting_longitude;
        }
        return $message;
    }

    function meetingLocation(&$message) {
// Returns an image button with a link to Google maps (should be to OSM)
        if (($this->item->meeting_latitude == 0) or ($this->item->meeting_longitude == 0)) {
            return false;
        } else {
            $target = "https://www.google.com/maps?q=";
            $target .= $this->item->meeting_latitude;
    ...
    function startDescription() {
        $message = $this->item->start_time;
        if ($message == "") {
            return false;
        }
        $message .= ", " . $this->item->start_details;
        $message .= ", " . $this->item->start_postcode . ',';
        $message .= ", " . $this->item->start_gridref;
        $message .= $this->getStreetmapLink($this->item->start_gridref, $this->item->start_latitude, $this->item->start_longitude);
        if ($this->item->start_latitude != 0) {
            $message .= "Lat: " . $this->item->start_latitude;
        }
        if ($this->item->start_longitude != 0) {
            $message .= ", Lon: " . $this->item->start_longitude;
        }
        return $message;
    }
...
// should return an image button with a link to OSM
        if (($this->item->start_latitude == 0) or ($this->item->start_latitude == 0)) {
            $message = "";
            return false;
        } else {
...
//            $target .= $this->item->start_latitude;
//            $target .= "&mlon" . $this->item->start_longitude . "&zoom=20";
//            echo "lat=" . $this->item->start_latitude . ", target=$target";
            $target = "https://www.google.com/maps?q=";
            $target .= $this->item->start_latitude;
            $target .= "," . $this->item->start_longitude;
            $message = $this->toolsHelper->imageButton("GO", $target, True);
...
// date is in format YYYY-MM-DD
        return substr($this->item->walk_date, 8, 2) . "/" . substr($this->item->walk_date, 5, 2) . "/" . substr($this->item->walk_date, 2, 2);
    }

    function get_walk_day() {
// Returns day of the week
        return substr($this->item->walk_date, 8, 2);
    }
...
//        echo $sql;
    ...
    function notify() {
//        echo "Target = " . $this->item->distance_miles . "<br>";
        $sql = "SELECT profiles.id,";
        $sql .= "profiles.min_miles, profiles.max_miles, ";
        $sql .= "profiles.home_group ";
...
//            echo $row->ra_group_code . ' ' . strpos($row->ra_group_code, $this->item->group_code) . '<br>';
            $result = strpos($row->ra_group_code, $this->item->group_code);
            if ($result === false) {
//                echo $row->ra_group_code . " / " . $this->item->group_code . " / " . $result . "<br>";
            } else {
...
//      Look up the profile record to ensure member has opted in
        $sql = 'SELECT contactviaemail from #__ra_profiles AS p ';
...
//        foreach ($followers as $follower) {
//            echo "Hi " . $follower->user_id . $follower->ra_display_name . $follower->member . $follower->email . ",<br>";
//        }
//        die('xx');
        ...
        switch ($mode) {
            case 'B':    //
                ...
                foreach ($followers as $follower) {
                    ...
                    $body = "Hi " . $follower->ra_display_name . ",<br>";
...
//                $body .= 'leader from our walking programme"</i>';
            ...
            case 'M':
                ...
                $body = '<b>' . $this->getTitle() . '</b><br>';
                $body .= Factory::getUser()->name . ' is sending this message to all other Followers:<br>';
...
//        echo $this->id . ', mode=' . $mode . ', to_user=' . $to_user . ', to_email=' . $to_email . '<br>';
         ...
         *  $sql = 'SELECT contact_by_email FROM #__ra_profiles where user_id=' . $user->id;
         * }
         */
